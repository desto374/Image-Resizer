...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "https://image-resizer-desto374s-projects.vercel.app",
    ...
    allow_origin_regex=r"^https://image-resizer-[a-z0-9-]+-desto374s-projects\.vercel\.app$",
...
#
# Directory where the static frontend files live. For Render deployments where the
# Root Directory is `Backend Folder-FastAPI`, put your HTML/CSS/JS and Image/ folder
...
#
# Where to send the user after successful auth. In production (Render serving the UI)
# this should be a same-origin path like "/Landing.html".
FRONTEND_REDIRECT_URL = os.getenv("FRONTEND_REDIRECT_URL", "/Landing.html")
...
# IMPORTANT: Ensure `index.html`, `Landing.html`, `login.html`, `signup.html`,
# `profile.html`, `styles.css`, `app.js`, `auth.js`, `profile.js`, `assets.js`, and the
# `Image/` folder are deployed inside `Backend Folder-FastAPI/`.
if (FRONTEND_DIR / "index.html").exists():
    app.mount("/", StaticFiles(directory=FRONTEND_DIR, html=True), name="frontend")
else:
    # If the frontend is not present in the deploy, keep the API running and make the
    # issue obvious instead of silently returning 404s.
    @app.get("/frontend-missing")
        ...
        return JSONResponse(
            {
                ...
                "hint": "Copy your *.html, *.js, styles.css, and Image/ folder into Backend Folder-FastAPI/ before deploying to Render.",
            },
            status_code=500,
        )
